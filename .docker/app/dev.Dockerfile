...
RUN --mount=type=cache,sharing=shared,id=composer_cache,target=/.composer_cache composer install --optimize-autoloader
...
RUN APP_ENV=dev /app/bin/console -vvv cache:warmup
